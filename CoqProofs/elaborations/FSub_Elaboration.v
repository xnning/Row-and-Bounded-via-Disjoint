...
Ltac gather_atoms ::=
  ...
  let E3 := gather_atoms_with (fun x : exp => fv_te x) in
  let E4 := gather_atoms_with (fun x : exp => fv_ee x) in
  let E5 := gather_atoms_with (fun x : typ => fv_tt x) in
  let E6 := gather_atoms_with (fun x : env => dom x) in
  constr:(A \u B \u C3 \u C4 \u D4 \u D5 \u D6 \u D16 \u D19 \u E1 \u E2 \u E3 \u E4 \u E5 \u E6).

(** * Translation of Type *)

Inductive fsub_trans_typ : env -> typ -> sty -> Prop :=
| fsub_trans_typ_top : forall FF,
    fsub_trans_typ FF typ_top sty_top
| fsub_trans_typ_nat : forall FF,
    fsub_trans_typ FF typ_nat sty_nat
| fsub_trans_typ_fvar_bind_sub_eq : forall FF a A B,
    binds a (bind_sub A) FF ->
    fsub_trans_typ FF A B ->
    fsub_trans_typ FF (typ_fvar a) (sty_and (sty_var_f a) B)
| fsub_trans_typ_arrow : forall FF A A' B B',
    fsub_trans_typ FF A A' ->
    fsub_trans_typ FF B B' ->
    fsub_trans_typ FF (typ_arrow A B) (sty_arrow A' B')
| fsub_trans_typ_all : forall L FF A B C,
    ...
    fsub_trans_typ FF (typ_all A B) (sty_all sty_top C)
| fsub_trans_typ_rcd_nil : forall FF,
    fsub_trans_typ FF (typ_rcd_nil) (sty_top)
| fsub_trans_typ_rcd_cons : forall FF i A' B' A B,
    fsub_trans_typ FF A A' ->
    fsub_trans_typ FF B B' ->
    rt_type B ->
    fsub_trans_typ FF (typ_rcd_cons i A B) (sty_and (sty_rcd i A') B')
.
...
(** * Translation of Typing *)

Inductive label_notin : i -> typ -> Prop :=
| label_notin_nil : forall i,
    label_notin i (typ_rcd_nil)
| label_notin_cons : forall i j A B,
    i <> j ->
    label_notin i B ->
    label_notin i (typ_rcd_cons j A B)
.


Inductive typing : env -> exp -> typ -> sexp -> Prop :=
  | typing_top : forall E,
      wf_env E ->
      typing E exp_top typ_top sexp_top
  | typing_lit : forall i E,
      wf_env E ->
      typing E (exp_lit i) typ_nat (sexp_lit i)
  | typing_var : forall E x T,
      wf_env E ->
      binds x (bind_typ T) E ->
      typing E (exp_fvar x) T (sexp_var_f x)
  | typing_abs : forall L E V e1 T1 EE A,
      (forall x : atom, x `notin` L ->
        typing (x ~ bind_typ V ++ E) (open_ee e1 x) T1 (open_sexp_wrt_sexp EE (sexp_var_f x))) ->
      fsub_trans_typ E (typ_arrow V T1) A ->
      typing E (exp_abs V e1) (typ_arrow V T1) (sexp_anno (sexp_abs EE) A)
  | typing_app : forall T1 E e1 e2 T2 EE1 EE2,
      typing E e1 (typ_arrow T1 T2) EE1 ->
      typing E e2 T1 EE2 ->
      typing E (exp_app e1 e2) T2 (sexp_app EE1 EE2)
  | typing_tabs : forall L E V e1 T1 EE A,
      fsub_trans_typ E V A ->
      (forall X : atom, X `notin` L ->
                   typing (X ~ bind_sub V ++ E) (open_te e1 X)
                          (open_tt T1 X)
                          (open_sexp_wrt_sty EE (sty_var_f X))) ->
      typing E (exp_tabs V e1) (typ_all V T1) (sexp_tabs sty_top EE)
  | typing_tapp : forall T1 E e1 T T2 EE A1 A2,
      typing E e1 (typ_all T1 T2) EE ->
      fsub E T T1 ->
      fsub_trans_typ E T A1 ->
      fsub_trans_typ E (open_tt T2 T) A2 ->
      typing E (exp_tapp e1 T) (open_tt T2 T) (sexp_anno (sexp_tapp EE A1) A2)
  | typing_sub : forall S E e T EE A,
      typing E e S EE ->
      fsub E S T ->
      fsub_trans_typ E T A ->
      typing E e T (sexp_anno EE A)
  | typing_rcd_nil : forall E,
      wf_env E ->
      typing E (exp_rcd_nil) typ_rcd_nil (sexp_top)
  | typing_rcd_cons : forall E i e1 e2 T1 T2 EE1 EE2,
      typing E e1 T1 EE1 ->
      typing E e2 T2 EE2 ->
      rt_expr e2 ->
      rt_type T2 ->
      label_notin i T2 ->
      typing E (exp_rcd_cons i e1 e2) (typ_rcd_cons i T1 T2)
               (sexp_merge (sexp_rcd i EE1) EE2)
  | typing_rcd_proj : forall E i e1 T1 T2 EE1 A,
      typing E e1 (typ_rcd_cons i T1 T2) EE1 ->
      fsub_trans_typ E T1 A ->
      typing E (exp_rcd_proj e1 i) T1
               (sexp_proj (sexp_anno EE1 (sty_rcd i A)) i)
.
...
Hint Extern 1 (wf_typ ?E ?T) =>
   match goal with
  | H: typing E _ T _ |- _ => apply (proj2 (proj2 (typing_regular _ _ _ _ H)))
   end.
...
match goal with
| H: typing _ ?e _ _ |- _ => apply (proj1 (proj2 (typing_regular _ _ _ _ H)))
end.
...
Fixpoint typ_size (t:typ) :=
  match t with
    ...
    | typ_bvar _ => 1
    | typ_fvar _ => 1
    | typ_arrow a b => 1 + typ_size a + typ_size b
    | typ_all a b => 1 + typ_size a + typ_size b
    | typ_rcd_nil => 1
    | typ_rcd_cons i a b => 1 + typ_size a + typ_size b
  end.

Fixpoint env_size (e:env) :=
  match e with
  | nil => 0
  | ( (X, bind_sub T) :: E) => typ_size T + env_size E
  | ( (X, bind_typ T) :: E) => typ_size T + env_size E
  end.
...
Lemma typ_size_open_tt_rec : forall T n X,
 typ_size (open_tt_rec n (typ_fvar X) T) = typ_size T.
Proof with eauto.
  induction T; introv; simpls...
  case_if...
Qed.

Lemma typ_size_open_tt : forall T X,
 typ_size (open_tt T (typ_fvar X)) = typ_size T.
Proof with eauto.
  unfold open_tt.
  intros.
  apply typ_size_open_tt_rec...
Qed.
...
Lemma wf_typ_renaming : forall F Q E Z T Y,
  ...
  Y \notin dom F \u dom E ->
  wf_typ (map (subst_tb Z (typ_fvar Y)) F ++ Y ~ bind_sub Q ++ E)
         (subst_tt Z (typ_fvar Y) T).
Proof with eauto.
  introv WF NOTIN.
...
Lemma fsub_trans_typ_renaming : forall F Z Q E T A Y,
  ...
  fsub_trans_typ (map (subst_tb Z (typ_fvar Y)) F ++ Y ~ bind_sub Q ++ E)
                 (subst_tt Z (typ_fvar Y) T)
...
Proof.
  intros.
  rewrite_env (map (subst_tb Z (typ_fvar Y)) nil ++ Y ~ bind_sub Q ++ E).
...
Proof with auto.
  ...
  -
    ...
    constructor...
    forwards ~ (A & I3) : H0 X... rewrite typ_size_open_tt. Omega.omega.
...
Proof with eauto.
  ...
  inverts TRS.
  inverts TRT.
  pick fresh X and apply S_forall; auto.
  eapply H0...
...
Lemma trans_subst_fsub_general : forall τ2 E σ τ A1 A2 D a DD F,
    ...
    sub DD (subst_sty_in_sty D a A1) A2
    /\ sub DD A2 (subst_sty_in_sty D a A1).
Proof with eauto.
  ...
  assert (I1 : fsub_trans_typ (map (subst_tb a σ) F ++ E) τ B).
    ...
    apply wf_env_append_invert in WFE...
    inverts WFE...
  forwards ~ : trans_fsub SUB...
  assert(I1: fsub_trans_typ E τ B).
    apply fsub_trans_typ_strengthening_head with (F ++ a ~ bind_sub τ);
    simpl_env...
    apply wf_env_append_invert in WFE...
    inverts WFE...
  ...
  *
  ...
  -
  simpls. inverts TR2.
  forwards ~ : IHTR1_1 WFE TRDD SUB TRD H2.
  destruct H.
  forwards ~ : IHTR1_2 WFE TRDD SUB TRD H4.
  destruct H1.
  split...
  ...
  *
  ...
  forwards ~ I4 : IHTR1_1 WFE TRDD SUB I1...
  destruct I4 as [I41 I42].
  forwards ~ I5 : IHTR1_2 WFE TRDD SUB TRD I2.
  destruct I5 as [I51 I52].
...
Lemma trans_subst_fsub : forall E τ2 σ τ A1 A2 C D a,
    ...
    sub D (open_sty_wrt_sty C A1) A2.
Proof with eauto.
...
Proof.
  ...
  inverts TRT. apply D_topR; auto. constructor.
  inverts TRT.
...
Lemma trans_typing : forall FF e T EE A D G,
    typing FF e T EE ->
    fsub_trans_D FF D ->
    fsub_trans_G FF G ->
    fsub_trans_typ FF T A ->
    has_type D G EE Inf A.
Proof with eauto.
  introv TY.
  generalize dependent D.
  ...
  - Case "sexp_app".
    ...
    assert (I2 : swft D B).
      ...
      inverts I2...
    apply T_app with (A1:=B)...
  - Case "sexp_tabs".
    inverts TRA.
    pick fresh X and apply T_tabs...
      eapply fsub_trans_G_swfe...
      pick fresh X.
      forwards ~ I1 : H0 X.
      forwards ~ [I _] : typing_regular I1.
      inverts I...
  - Case "sexp_tapp".
    forwards ~ (B & I) : fsub_trans_typ_exists E (typ_all T1 T2).
    forwards ~ : fsub_trans_typ_uniq TRA H1. substs.
    apply T_anno.
    forwards ~ : IHTY TRD TRG I. inverts I.
    apply T_sub with (open_sty_wrt_sty C A1)...
    + apply T_tapp with (B:=sty_top)...
      apply D_topR... constructor...
    + pick_fresh X.
      forwards ~ : trans_subst_fsub TRD H H0 H1.
  - Case "sexp_sub".
    forwards ~ : fsub_trans_typ_uniq H0 TRA. substs.
    apply T_anno.
    forwards ~ (B & I) : fsub_trans_typ_exists E S.
    forwards ~ : IHTY TRD TRG I.
    apply T_sub with B...
    eapply trans_fsub...
  - Case "rcd nil".
    inverts TRA...
  ...
  - Case "rcd perm".
    ...
    forwards ~ : IHTY (sty_and (sty_rcd i A0) B) TRD TRG.
    constructor...
    constructor...
