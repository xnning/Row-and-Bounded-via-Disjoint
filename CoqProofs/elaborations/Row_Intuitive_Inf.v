...
Require Export Row_Intuitive_Syntax.
Require Import Coq.Logic.FunctionalExtensionality.
Require Import Coq.Program.Equality.
Require Export Metalib.Metatheory.
Require Export Metalib.LibLNgen.

Fixpoint trans_rt (T:rt) : sty :=
  match T with
  | rt_Base => sty_nat
  | rt_Fun t1 t2 => sty_arrow (trans_rt t1) (trans_rt t2)
  | rt_ConQuan rl t => sty_all (trans_rlist rl) (trans_rt t)
  | rt_Record r => (trans_rtyp r)
  end

with

trans_rtyp (T:rtyp) : sty :=
  match T with
  | r_TyVar_b n => sty_var_b n
  | r_TyVar_f x => sty_var_f x
  | r_Empty => sty_top
  | r_SingleField l t => sty_rcd l (trans_rt t)
  | r_Merge r1 r2 => sty_and (trans_rtyp r1) (trans_rtyp r2)
  end

with

trans_rlist (T:rlist) : sty :=
  match T with
  | R_Empty => sty_top
  | R_Cons r rl => sty_and (trans_rtyp r) (trans_rlist rl)
  end.
...
Inductive degree_rexp_wrt_rtyp : nat -> rexp -> Prop :=
  ...
  | degree_wrt_rtyp_re_Lit : forall n1 x1,
    degree_rexp_wrt_rtyp n1 (re_Lit x1)
